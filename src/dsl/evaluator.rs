...
fn eval_filter_as_box(node: &Node<FilterExpr>) -> Result<Box<dyn Filter>> {
    match &node.value {
        ...
        FilterExpr::Regex { path, pattern } => Ok(Box::new(RegexFilter::new(path, pattern)?)),
        ...
        FilterExpr::IsNull(_path) => Err(MirrorMakerError::Config(
            "IS_NULL filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::IsNotNull(_path) => Err(MirrorMakerError::Config(
            "IS_NOT_NULL filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::IsEmpty(_path) => Err(MirrorMakerError::Config(
            "IS_EMPTY filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::IsNotEmpty(_path) => Err(MirrorMakerError::Config(
            "IS_NOT_EMPTY filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::IsBlank(_path) => Err(MirrorMakerError::Config(
            "IS_BLANK filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::StartsWith { path: _, prefix: _ } => Err(MirrorMakerError::Config(
            "STARTS_WITH filter not yet implemented in evaluator".to_string(),
        )),

        FilterExpr::EndsWith { path: _, suffix: _ } => Err(MirrorMakerError::Config(
